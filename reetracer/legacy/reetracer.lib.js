...
class SceneImporter {
    ...
    compatibilityMode() {
        ...
        for (var i in this.sceneDescription.objects) {
            ...
            // Loop through base properties
            for (var j in baseProperties) {
                var prop = baseProperties[j];
                if (material[j]) {
                    material[prop] = material[j];
                }
            }
            // Loop through texture properties
            for (j in textureProperties) {
                prop = textureProperties[j];
                if (material[j] && material[j].src) {
                    material[prop] = material[j];
                }
            }
        }
 ...
 */
class ReeTracerJs {
    constructor(canvasId, sceneDescription) {
        var canvas = document.getElementById(canvasId);
        var renderSettings = sceneDescription.settings;
    ...
    render(callback) {
        var worker = new Worker('legacy/reetracer.worker.js');
        var self = this;
